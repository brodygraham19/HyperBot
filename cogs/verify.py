...
class Verify(commands.Cog):
    ...
    async def verify_user(self, i: discord.Interaction, user: discord.Member):
        role = i.guild.get_role(int(VERIFY_ROLE_ID)) if VERIFY_ROLE_ID and VERIFY_ROLE_ID.isdigit() else None
        if not role: return await i.response.send_message("❌ Verify role not configured.", ephemeral=True)
        await user.add_roles(role, reason=f"Manual verify by {i.user}")
        await i.response.send_message(f"✅ {user.mention} verified.")
        await self._log(i.guild, discord.Embed(description=f"✅ **Verified:** {user.mention}", color=0x57F287))
