...
WELCOME_DM_TEXT = os.getenv("WELCOME_DM_TEXT", "Welcome to the server! Please verify to unlock channels.")
...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        if WELCOME_CHANNEL_ID and WELCOME_CHANNEL_ID.isdigit():
            ...
            if ch:
                try: await ch.send(f"👋 Welcome {member.mention}! {WELCOME_DM_TEXT}")
                except: pass
        if WELCOME_DM_ENABLED:
            try: await member.send(WELCOME_DM_TEXT)
            except: pass
