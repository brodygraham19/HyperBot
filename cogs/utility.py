...
class Utility(commands.Cog):
    ...
    async def helpcmd(self, i: discord.Interaction):
        embed = discord.Embed(title="HyperBot V3 — Commands", color=0x2b2d31, description=(
    ...
    async def serverinfo(self, i: discord.Interaction):
        ...
        if g.owner: embed.add_field(name="Owner", value=g.owner.mention, inline=False)
        await i.response.send_message(embed=embed, ephemeral=True)

    @app_commands.command(name="userinfo", description="Show user info")
    async def userinfo(self, i: discord.Interaction, user: discord.Member):
        embed = discord.Embed(title=str(user), color=user.top_role.color if user.top_role else 0x2b2d31)
        if user.joined_at: embed.add_field(name="Joined", value=discord.utils.format_dt(user.joined_at, style='R'))
